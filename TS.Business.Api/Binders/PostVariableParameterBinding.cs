...
namespace TS.Business.Api.Binders
{
    /// <summary>
    /// Custom binding to support multiple parameters in webapi POST requests
    /// </summary>
    public class PostVariableParameterBinding : HttpParameterBinding
    {
        ...
        private const string MultipleBodyParametersRaw = "MultipleBodyParametersRaw";
        private const bool AllowJsonContentType = true;
        ...
        {
            //To see whether the action is flaged or not
        ...
        object StringToType(string stringValue)
        {
            ...
            else if (Descriptor.ParameterType == typeof(TimeSpan)) value = TimeSpan.Parse(stringValue, CultureInfo.CurrentCulture);
            else if (Descriptor.ParameterType == typeof(TimeSpan?)) value = string.IsNullOrWhiteSpace(stringValue) ? (TimeSpan?)null : TimeSpan.Parse(stringValue, CultureInfo.CurrentCulture);
